...
func (h *ItemUsecase) CreateItem(title string, explanation string, priceStr string, file multipart.File, fileHeader *multipart.FileHeader, uid string) (map[string]string, string, error) {
	...
	// 3. DAOの呼び出し（永続化）
	if err := h.postItemsDao.InsertItem(title, price, explanation, imagePath, uid); err != nil {
